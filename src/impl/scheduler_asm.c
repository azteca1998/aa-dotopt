...
static void impl_zz_zz_zz(matrix_t *a, matrix_t *b, matrix_t *c, int zero_fill)
{
    ...
    imts_init_child(&imts_l2[0], &imts_l3, 128, 4);
    imts_init_child(&imts_l2[1], &imts_l3, 128, 4);

    imts_init_child(&imts_l1[0], &imts_l2[0], 32, 2);
    imts_init_child(&imts_l1[1], &imts_l2[0], 32, 2);
    imts_init_child(&imts_l1[2], &imts_l2[0], 32, 2);
    imts_init_child(&imts_l1[3], &imts_l2[0], 32, 2);
    imts_init_child(&imts_l1[4], &imts_l2[1], 32, 2);
    imts_init_child(&imts_l1[5], &imts_l2[1], 32, 2);
    imts_init_child(&imts_l1[6], &imts_l2[1], 32, 2);
    imts_init_child(&imts_l1[7], &imts_l2[1], 32, 2);
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        imts_init_child(&thread_data[i].imts, &imts_l1[i % 8], 8, 0);
...
static void *run_zz_zz_zz(void *ptr)
{
    ...
    {
        if (m + data->imts.tile_size <= data->imts.until_m
            && k + data->imts.tile_size <= data->imts.until_k
            && n + data->imts.tile_size <= data->imts.until_n)
        {
            impl_ptr = zero_fill
            ...
            (*impl_ptr)(
                &zorder_at(data->a->data, m, k),
                &zorder_at(data->b->data, k, n),
                &zorder_at(data->c->data, m, n)
            );
        }
        else
        {
            ta.data = &zorder_at(data->a->data, m, k);
            tb.data = &zorder_at(data->b->data, k, n);
            tc.data = &zorder_at(data->c->data, m, n);
...
const impl_scheduler_asm_t impl_scheduler_asm[28] = {
    &impl_abort,
    &impl_abort,
    &impl_abort,
    &impl_abort,
